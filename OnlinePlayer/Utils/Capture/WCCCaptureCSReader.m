//
//  WCCCaptureCSReader.m
//  wochacha
//
//  Created by kun zhang on 13-11-15.
...
@implementation WCCCaptureCSReader
@synthesize
    ...
    if(data) {
        ...
        if (result == 2 && self.strBarcode && _iBarType>=8 && _iBarType<=14) {//only product code
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.delegate && [self.delegate respondsToSelector:@selector(captureReader:didFindCSGoodsBarcode:withImage:)]) {
                    [self.delegate captureReader:self didFindCSGoodsBarcode:self.strBarcode withImage:img];
                }
            });
        }
    }
